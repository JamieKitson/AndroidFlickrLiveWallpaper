...
namespace FlickrLiveWallpaper
{
    public class MyFlickr
    {
        ...
        const int BATCH_SPAN_DAYS = 7;
        const int BATCH_SIZE = 100;
        public const string HIGHRES = "__highres";
        ...
        */
        ...
        public static async Task Upload()
        {
            try
            {
                ...
                Flickr f = MyFlickr.getFlickr();
                IOrderedEnumerable<StorageFile> pics;

                do
                {
                    pics = await GetFileBatch();

                    foreach (StorageFile p in pics)
                    {
                        ...
                        {
                            Settings.DebugLog("Found photo(s)");
        ...
        */
        ...
        public static async Task<string> UploadPictureAsync(Stream stream, string filename, string title, string description, string tags, bool isPublic, bool isFamily, bool isFriend, ContentType contentType, SafetyLevel safetyLevel, HiddenFromSearch hiddenFromSearch)
        {
            ...
            parameters.Add("oauth_consumer_key", FlickrKeys.APIKey);
        ...
        {
            var key = FlickrKeys.APIKey + "&" + tokenSecret;
